...
function fetchConfigs() {
  ...
  return Request.request('configurations').then(
    (configs: any) => {
...
function fieldSet(id, legend, options) {
  ...
  for (const { label, value, description } of options) {
    const inp = Element.create({ tag: 'input', id: value, value }),
      lab = Element.create({ tag: 'label', innerHTML: label });
...
export function dialog() {
  return fetchConfigs().then((configSelect) => {
    const publicSelect = fieldSet('public-select', 'game type', [
      ...
      createButton = Element.create({tag: 'button', innerHTML: 'create', className: 'uibutton ok-b'}),
      cancelButton = Element.create({tag: 'button', innerHTML: 'cancel', className: 'uibutton cancel-b'}),
    ...
    createButton.addEventListener('click', () => {
      const conf = configSelect.value(),
        psv = publicSelect.value();
